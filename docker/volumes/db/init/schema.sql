...
CREATE POLICY privacy_settings_select ON public.privacy_settings FOR SELECT TO authenticated USING ((SELECT auth.uid()) = user_id);
CREATE POLICY privacy_settings_insert ON public.privacy_settings FOR INSERT TO authenticated WITH CHECK ((SELECT auth.uid()) = user_id);
CREATE POLICY privacy_settings_update ON public.privacy_settings FOR UPDATE TO authenticated USING ((SELECT auth.uid()) = user_id) WITH CHECK ((SELECT auth.uid()) = user_id);
...
BEGIN
  ...
  SELECT * INTO result FROM public.privacy_settings WHERE user_id = auth.uid();
  RETURN result;
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.privacy_settings_lww_upsert(jsonb, timestamp with time zone) TO authenticated;
