...
import { getDownloadSignedUrl, isUserBookObjectKey } from '@/utils/object';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (!fileKey || typeof fileKey !== 'string' || !isUserBookObjectKey(fileKey, user.id)) {
    ...
    if (req.method === 'POST') {
      ...
      if (!fileKeys.every((key) => isUserBookObjectKey(key, user.id))) {
        return res.status(400).json({ error: 'All fileKeys must be valid user book keys' });
      }
