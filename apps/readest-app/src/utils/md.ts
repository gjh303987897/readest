...
// Section tracking keeps work grouped per chapter, matching fb2.js.
...
// Keep Markdown reader options scoped to this parser so importing `marked`
// elsewhere cannot change how book sections and footnotes are rendered.
...
export async function makeMarkdownBook(file: File): Promise<BookDoc> {
  ...
  // Markdown uses the same display-transform pipeline as EPUB/MOBI. The
  // paginator renders `loadContent()` via srcdoc, with transformed content
  // cached until the section is unloaded. `createDocument()` stays raw so
  // display transforms are applied exactly once.
  ...
  const transformSection = async (index: number): Promise<string> => {
    ...
    try {
      ...
      // Readonly, mirroring foliate's Loader.createURL dispatch.
