...
import { getStoredBackendConnection } from './backendEndpoint';
...
export const getBaseUrl = () =>
  getStoredBackendConnection()?.apiBaseUrl ??
...
export const getAPIBaseUrl = () => {
  const configured = getStoredBackendConnection();
  if (configured) return `${configured.apiBaseUrl}/api`;
