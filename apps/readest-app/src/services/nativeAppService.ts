...
import { copyURIToPath, hasAmbientLightSensor } from '@/utils/bridge';
import { detectViewTransitionGroup, detectViewTransitionsAPI } from '@/utils/viewTransition';

import { BaseAppService } from './appService';
import { DatabaseOpts, DatabaseService } from '@/types/database';
import { SchemaType } from '@/services/database/migrate';
import { DATA_SUBDIR, LOCAL_BOOKS_SUBDIR, SETTINGS_FILENAME } from './constants';
...
const getPathResolver = ({
  ...
  const getCustomBasePrefixSync = isCustomBaseDir
    ? (baseDir: BaseDir) => {
        return () => {
          const dataDirs = ['Settings', 'Data', 'Books'];
...
const IS_APP_STORE_BUILD = process.env['NEXT_PUBLIC_DIST_CHANNEL'] === 'appstore';
...
export class NativeAppService extends BaseAppService {
  ...
  override canCustomizeRootDir = !IS_APP_STORE_BUILD;
  ...
  override canReadExternalDir = !IS_APP_STORE_BUILD;
