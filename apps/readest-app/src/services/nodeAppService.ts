...
import { DATA_SUBDIR, LOCAL_BOOKS_SUBDIR } from './constants';
...
// When customRootDir is set, Settings/Data/Books all resolve under it.
...
const getPathResolver = ({ customRootDir }: { customRootDir?: string } = {}) => {
  ...
  const getCustomBasePrefix = isCustomBaseDir
    ? (base: BaseDir) => {
        const dataDirs: BaseDir[] = ['Settings', 'Data', 'Books'];
