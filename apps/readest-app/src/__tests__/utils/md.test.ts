...
// MD books get the same transformTarget 'data' pipeline as EPUB/MOBI, so
// display transforms apply to Markdown books too. The paginator renders `loadContent()` via
...
describe('makeMarkdownBook display transforms', () => {
  ...
  it('routes section loadContent through data listeners with the section index as name', async () => {
    ...
    // The event name is the bare section index used by the transform context.
  ...
  it('caches the transformed content until unload invalidates it', async () => {
    ...
    // Unloading a view invalidates its cached transformed content.
    book.sections[0]!.unload!();
    await book.sections[0]!.loadContent!();
    expect(listener).toHaveBeenCalledTimes(2);
  });

  it('keeps createDocument raw so display transforms do not double-apply', async () => {
    const book = await make('# A\n\nteh text\n');
